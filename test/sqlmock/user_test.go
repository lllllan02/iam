...
func TestUserData_Create(t *testing.T) {
	...
	mock.ExpectQuery("SELECT").WithArgs(user.Email).WillReturnRows(sqlmock.NewRows([]string{"count(*)"}))
	mock.ExpectExec("INSERT INTO").WithArgs(user.InstanceID, sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), user.Username, encrypt.Encrypt(user.Password), user.Email).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	assert.NoError(t, userData.Create(ctx, user))
